...
icd10_ccs = pickle.load(open('icd10_to_ccs10.pkl','rb'))
...
def diag_prediction_mimic4_fn(patient: Patient):
    ...
    for i in range(len(visit_ls)):
        ...
        for con in conditions:
            ...
            if con in icd10_ccs:
                cond_ccs += icd10_ccs[con]
            elif mapping3.map(con):
...
class MMDataset(Dataset):
    def __init__(self, dataset, tokenizer, dim, device, trans_dim=0, di=False):
        ...
        print(len(self.sequence_dataset))

        # self.graph_data = pickle.load(open('log/graph_data.pkl', 'rb'))
        ...
        self.graph_data = self.sequence_dataset

        # with open('logs/graph_data.pkl', 'wb') as f:
    ...
    def __getitem__(self, idx):
        ...
        return sequence_data, graph_data, idx